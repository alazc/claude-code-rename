...
const FAKE_CWD = "C:\\fake\\project\\path"; // matches fixture
const FAKE_NEW = "C:\\fake\\new-path"; // arbitrary
const FAKE_ENCODED = "C--fake-project-path"; // matches fixture
const FAKE_NEW_ENCODED = "C--fake-new-path";
...
describe("cli: pre-flight gates", () => {
  ...
  it("encoder sanity mismatch → exit 4", async () => {
    ...
    const projectsDir = path.join(root, "projects");
    // Choose oldPath so encode(oldPath) is the actual folder name.
    const oldPath = "C:\\foo";
    const folderName = "C--foo"; // encodePath("C:\\foo") on Windows or POSIX
    const wrongFolder = path.join(projectsDir, folderName);
    await fsp.mkdir(wrongFolder, { recursive: true });
    // jsonl cwd encodes to "C--bar" — does NOT match folder name "C--foo".
    await fsp.writeFile(
      path.join(wrongFolder, "a.jsonl"),
      JSON.stringify({ event: "session-start", cwd: "C:\\bar" }) + "\n",
    );
    const newPath = "C:\\baz";
    const { io, stderr } = makeIO();
    const code = await main(
  ...
  it("OLD literal not in jsonl → exit 4", async () => {
    ...
    // (C--fake-project-path) but does NOT appear as a literal anywhere in the
    // transcripts. encodePath collisions: any of `:_/\\ ` map to `-`, so
    // `C:\\fake-project-path` encodes the same as `C:\\fake\\project\\path`.
    const oldPathAlt = "C:\\fake-project-path";
    const { io, stderr } = makeIO();
    const code = await main(
