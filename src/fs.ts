...
async function* walkJsonlImpl(
  ...
  // Cycle dedup at the start of every recursion (NOT only on the
  // symlink-to-dir branch) — a cycle of dir-symlinks `A -> B -> A` would
  // otherwise traverse the whole cycle once before dedup fires, yielding
  // each contained file twice. Stat the dir, key on (dev, ino), and skip
  // re-entry. Stat-failure (e.g. permissions) is treated as skip-silently
  // like the rest of the walker.
  ...
  for (const entry of entries) {
    ...
    if (resolved.isDirectory()) {
      // walkJsonlImpl's own start-of-function dedup catches the cycle case.
      yield* walkJsonlImpl(full, visited);
      continue;
    }
    if (resolved.isFile()) {
